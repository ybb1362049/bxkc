 ...
 *
 */
public class DX000583_List extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == rsp) {
			try {
				...
				// 具体赋值
				String syntaxOfRows = "{div[class=news-list]>ul[class=list]}|||{li>a[href]}::<2";// span:matches(20\\d{2}-\\d{1,2}-\\d{1,2})
				...
				String regexOfPage = "div[class=fy]:has(form)";
				...
				if (regexOfPage.equals("") && theLastRowPage != null) {
					...
					Elements eles = parseSelector(document, regexOfPage, false, 1);
					if (eles != null && eles.size() > 0) {
						allFatherTagOfPage = eles.first();
					}
				}
						...
						if (pageATags != null) {
							for (int i = 0; i < pageATags.size(); i++) {
								if (pageATags.get(i).text().matches("\\d{1,3}")) {
									nextPageNum = new Integer(
											pageATags.get(i).text().replaceAll("(\\D{0,2})(\\d{1,3})(\\D{0,2})", "$2"));
	...
	public static Elements parseSelector(Element element, String syntax, boolean returnAllElement, int regPartIndex)
		...
		Elements regParts = new Elements();
		Element regPart = null;
		Elements allEles = new Elements();
		Element eleToSelect = null;

		String headSyntax = null;
		String tailSyntax = null;
		if (syntax.contains("|||")) {
			headSyntax = syntax.substring(0, syntax.lastIndexOf("|||"));
			tailSyntax = syntax.substring(syntax.lastIndexOf("|||") + 3, syntax.length());
		} else {
			tailSyntax = syntax;
		}

		if (headSyntax != null && !headSyntax.equals("")) {
			headSyntax = headSyntax.substring(tailSyntax.indexOf("{") + 1, headSyntax.lastIndexOf("}"));
		}

		// 解析syntax
		if (tailSyntax.contains("::")) {
			syntaxLeft = tailSyntax.substring(tailSyntax.indexOf("{") + 1, tailSyntax.lastIndexOf("}"));
			syntaxRight = tailSyntax.substring(tailSyntax.lastIndexOf("::") + 2, tailSyntax.length());
		...
		Elements headElements = null;
		if(headSyntax != null && !headSyntax.equals("")){
			headElements = parseSelector(element,headSyntax,true,1);
			if(headElements!=null){
				for(Element ele:headElements){
					eleToSelect = ele;
					if (eleToSelect != null && syntaxLeft != null && !syntaxLeft.equals("")) {
						Elements elees = eleToSelect.select(syntaxLeft);
						if(elees!=null&&elees.size()>0){
			...
			eleToSelect = element;
			if (eleToSelect != null && syntaxLeft != null && !syntaxLeft.equals("")) {
				regParts = eleToSelect.select(syntaxLeft);
			} else {
				throw new Exception("syntaxLeft = null or syntaxLeft = \"\"");
			}
		}
