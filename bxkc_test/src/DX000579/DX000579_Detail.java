...
public class DX000579_Detail extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				String remove = "div#about_ctl00_proInfo div.con-bot,-1";
				boolean getdate = false;
				String dateString = "";

				Document xml = Jsoup.parse(originTxtRspContent);
				Element titElement = xml.select("div#about_ctl00_proTit").first();
				Element content = xml.select("div.con").first();
