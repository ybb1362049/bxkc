...
public class XX0948_Detail extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				String content = "";
				Element div = doc.select("div[portletmode=simpleArticleAttri]").first();
				if (div != null) {
					div.select("script").remove();
					div.select("table").get(1).remove();
