...
public class GJ_02344_Detail extends TxtRspHandler {
	...
	/* (non-Javadoc)
	 * @see cn.internetware.phone.extension.response.impl.TxtRspHandler#processTxtRspContent(cn.internetware.phone.extension.response.RspState, java.lang.String)
	 */
	@Override
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element titleElement = xml.select("td[height=38]").first();
				String title = titleElement.text().trim();
				response.title = title;
				Element contentElement = xml.select("body > div.body > div.right > div.nyxwlb > div > table > tbody > tr:nth-child(4) > td").first();
				contentElement.select("*[style~=^.*display\\s*:\\s*none\\s*(;\\s*[0-9A-Za-z]+|;\\s*)?$]").remove();
				contentElement.select("script").remove();
				
				Element dateElement = xml.select("body > div.body > div.right > div.nyxwlb > div > table > tbody > tr:nth-child(3) > td").first();
				...
				if (contentElement.select("a") != null) {
					...
					for (Element href : as) {
						...
						if (!hre.contains("@") && !"".equals(hre) && !hre.contains("javascript") && !hre.contains("http")) {
							hre = "http://www.keypower8888.com" + hre;
							href.attr("href", hre);
						}
					}
				}
				if (contentElement.select("img").first() != null) {
					...
					for (Element imgs : img) {
						...
						if (!imgss.contains("javascript") && !"".equals(imgss) && !imgss.contains("http")) {
							imgss = "http://www.keypower8888.com" + imgss;
							imgs.attr("src", imgss);
