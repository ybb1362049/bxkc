...
public class SJ_04339_List extends TxtRspHandler {
...
//				Element table = xml.select("div[class=list clearfix ]").first();
				Element table = xml.select("ul").last();
...
//					String title = ae.text().trim().replace("...", "");
					String title = ae.text().trim().replace("...", "");
					String date = tre.select("span[class=date]").first().text().trim();
...
//				Element str = xml.select("div[class= comWidth clearfix paging]").last();
