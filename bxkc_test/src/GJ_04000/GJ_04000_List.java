...
public class GJ_04000_List extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState,
		...
		if (rspState == RspState.Login) {
			try {
				...
				NodeList table_list=xmlDoc.getElementsByTagName("td");
				for(int i=0;i<table_list.getLength();i++){
					Element table_ele=(Element)table_list.item(i);
					if("85%".equals(table_ele.getAttribute("width"))){
						Element tbody=(Element) table_ele.getElementsByTagName("table").item(0);
						//Element td_ele=(Element)table_ele.getElementsByTagName("td").item(0);
						NodeList lists=tbody.getElementsByTagName("tr");
						for(int j=0;j<lists.getLength();j++){
							...
							Element dates=(Element) lis.getElementsByTagName("span").item(0);
