...
public class DX000084_Detail extends TxtRspHandler {
	...
	String url = "http://www.crmb.net.cn";
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				String content = "";
				Element dive = doc.select("span[id=Lb_title]").first();
				{
					response.title=dive.text().replaceAll("\\s*", "");
				}
				Element div = doc.select("td[class=colSpan=1]").first();
				if (div != null) {
