 ...
 *
 */
public class QX_01421_List extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == rsp) {
			try {
				...
				// re_rows
				String syntaxOfRows = "tr:has(td:has(a[class])):has(td[class*=time])";// span:matches(20\\d{2}-\\d{1,2}-\\d{1,2})
				...
				OUTFOR: for (Element row : rows) {
					...
					// re_date
					String[] regexesOfDate = {"td[class*=timestyle]"};
					String date = null;
