...
public class DX000278_Detail extends TxtRspHandler {
		...
		if (rspState == RspState.Login) {
			try {
				...
				for (int i = 0; i < td_list.getLength(); i++) {
					...
					// 判断目的td
					if ("ConpageWrap".equals(td_point.getAttribute("class"))) {
					...
					//	response.title=h2.getTextContent().trim();
						...
						// 将整个td的内容存放到字符串
						if ("ContentContarinterWrap".equals(td_point
								.getAttribute("class"))) {
