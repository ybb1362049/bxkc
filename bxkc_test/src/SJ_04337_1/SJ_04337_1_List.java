...
public class SJ_04337_1_List extends TxtRspHandler {
...
//					String title = ae.text().trim().replace("...", "");
					String title = ae.text().trim().replace("...", "");
					String date = tre.select("span[style=color:#AAA;font-size:12px;]").first().text().trim().replace("[", "").replace("]", "");
				...
				Element str = xml.select("span[style=float: right; height: 30px; line-height: 30px;]").first();
