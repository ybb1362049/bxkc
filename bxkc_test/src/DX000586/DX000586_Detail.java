...
public class DX000586_Detail extends TxtRspHandler {
	...
	private static String url = "http://www.bdwgs.com";
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				String content = "";
				Element dive = doc.select("div.hh1").first();
				if(dive!=null)
				{
					content=dive.outerHtml();
					response.title=dive.text().replaceAll("\\s*", "");
				}
				Element div = doc.select("ul.uul").first();
				if (div != null) {
