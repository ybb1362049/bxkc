...
public class XX1298_Detail extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState repState, String originTxtRspContent) {
		...
		if (repState.equals(RspState.Login)) {
			try {
				...
				Element content = doc.select("div.div_content").first();
				if(content != null){
					content.select("div[style=text-align:center;color:#999999;font-size:14px;margin-top:10px;]").remove();
					...
					Element title = doc.select("div.div_title").first();
					title.select("div.box").remove();
					response.content = title.outerHtml()+content.outerHtml();
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public String completionLinksHostName(String link,String hostName){//hostname 类似这样的形式 http://www.hostname.com
		String complementedLink = "";
		if(link.indexOf("http") != 0 && (link.indexOf("mail") != 0||link.indexOf("data") != 0)){//判断是否需要补全链接
			if(link.indexOf("./") == 0)
				complementedLink = super.getContextInfo(Context.REQUEST_CONTEXT_PATH)+ link.substring(1);
				...
				complementedLink = super.getContextInfo(Context.REQUEST_CONTEXT_PATH)+link;
