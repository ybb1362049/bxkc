...
public class DX000598_Detail extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element titleElement = xml.select("div.biaoti-2").first();
				String title = titleElement.text().trim();
				response.title = title;
				Element contentElement = xml.select("div.nr_01").last();
				...
				if (contentElement.select("a").first() != null) {
					...
					for (Element href : as) {
						...
						if (!hre.contains("@") && !"".equals(hre) && !hre.contains("javascript")
								&& !hre.contains("http")) {
							hre = "http://www.gynybx.com.cn" + hre;
							href.attr("href", hre);
						}
					}
				}
				if (contentElement.select("img").first() != null) {
					...
					for (Element imgs : img) {
						...
						if (!imgss.contains("javascript") && !"".equals(imgss) && !imgss.contains("http")) {
							imgss = "http://www.gynybx.com.cn" + imgss;
							imgs.attr("src", imgss);
