...
public class DS_01100_Detail extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				String remove = "table[width=706],-1";
				boolean getdate = false;
				String dateString = "";

				Document xml = Jsoup.parse(originTxtRspContent);
				// Element titElement = xml.select("").first();
				Element content = xml.select("table[width=707][cellspacing=0]").first();
