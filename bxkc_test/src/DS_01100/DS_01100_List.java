...
public class DS_01100_List extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element content = xml.select("table[width=724][cellspacing=0]:has(a)").get(1);
				removeChild(content, remove);
				Elements list = content.select("tr:has(a):not(td[height=60])");

				for (Element value : list) {

					Element aElement = value.select("a").first();
					Element dateElement = value.select("td").get(2);
