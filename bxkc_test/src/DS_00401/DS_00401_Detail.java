...
public class DS_00401_Detail extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				String content = "";
				Element div = doc.select("div[class=border]").first();
				if (div != null) {
					div.select("script").remove();
					div.select("link").remove();
					div.select("tr").get(1).remove();
					Element tit = div.select("td[height=76]").first();
					if (tit != null) {
						response.title = tit.text().trim();
					}
