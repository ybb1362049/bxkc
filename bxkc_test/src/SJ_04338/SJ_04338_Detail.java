...
public class SJ_04338_Detail extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == rsp) {
			try {
				...
				Element pe = xml.select("div[class=content content_info]").first();
				Element h1s = xml.select("div[class=middle_r]").first();
				Element h2 = h1s.select("h2").first();
