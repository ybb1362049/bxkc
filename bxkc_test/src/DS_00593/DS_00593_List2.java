...
public class DS_00593_List2 extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element list_ee = document.select("div.infotexta_1 ul").first();
				...
				for (Element li : li_ee) {
					...
					Element title_ee = a_ee.select("span").first();
					title = title_ee.attr("title").trim().replaceAll("[ |\u3000]", "");
				...
				if (page_ee != null) {
					...
					if (m.find()) {
						...
						// pageCount = (int)
						// Math.ceil(Integer.parseInt(pageCount) / 20.0) + "";
