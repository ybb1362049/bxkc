...
public class QX_01614_Detail extends TxtReqRspHandler {
	...
	private static String url = "http://www.shuicheng.gov.cn";
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				String content = "";
				Element td = doc.select("div.title").first();
				if (td != null) {
					content = td.outerHtml();
					response.title = td.text().trim();
				}
				Element time = doc.select("div.toolbar").first();
				if (time != null) {
					Matcher m = p.matcher(time.outerHtml());
					if (m.find()) {
						response.date = m.group();
					}
				}
				Element div = doc.select("div.content").last();
				if (div != null) {
	...
	public IwResponse sendIwRequest(IwRequest iwRequest) {
		...
		try {
			...
			String detailUrl = "http://www.aqzbcg.org/Front_qs/InfoDetail/?InfoID=" + InfoID + "&CategoryNum="
					+ CategoryNum;
