...
public class DX000282_Detail extends TxtRspHandler {
		...
		if (rspState == RspState.Login) {
			try {
				...
				for (int i = 0; i < td_list.getLength(); i++) {
					...
					// 判断目的td
					if ("container fn-clear".equals(td_point.getAttribute("class"))) {
						Element h2 = (Element) td_point.getElementsByTagName(
								"h2").item(0);
						response.title=h2.getTextContent().trim();
						content = Utils.getNodeHtml(h2);
					}
						// 将整个td的内容存放到字符串
						if ("info".equals(td_point
								.getAttribute("class"))) {
							...
							if (IMG_list != null) {
								for (int j = 0; j < IMG_list.getLength(); j++) {
									...
									if (src.indexOf("http") == -1) {
										src = "http://erdos.app.yuecai.com"+src;
									}
							...
							if (a_list != null) {
								for (int j = 0; j < a_list.getLength(); j++) {
									...
									if (href.indexOf("www") == -1
										...
										href = "http://erdos.app.yuecai.com"+href;
									}
