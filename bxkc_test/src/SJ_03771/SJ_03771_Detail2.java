...
public class SJ_03771_Detail2 extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element detailEle = document.select("div.content-list").first();
				detailEle.select("script").remove();
				detailEle.select("div.lc-title-s").remove();
				Element titleEle = detailEle.select("div.lc-title").first();
