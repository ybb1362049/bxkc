...
public class DS_01418_List extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState arg0, String arg1) {
		...
		if (arg0 == RspState.Login) {
			try {
				...
				Elements lis = xml.select("ul.list li");
				for (Element li : lis) {
					if (li.text() == null || "".equals(li.text())) {
						continue;
					}
					...
					String date = li.select("em").first().text().trim();
