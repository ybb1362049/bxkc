...
public class DX000588_List extends TxtRspHandler {

	private static SimpleDateFormat newsdf = new SimpleDateFormat("yyyy-MM-dd");
	private static Pattern p_page = Pattern.compile("(?<=page=)(\\d+)");
	private Matcher m;
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element list_ee = document.select("div.content").last();
				Elements li_ee = list_ee.select("table");
...
//					title = a_ee.text().trim().replaceAll("[\\ |\u3000]", "");
				...
				Element page_ee = document.select("div.page a").last();
				if (page_ee != null) {
					m = p_page.matcher(page_ee.attr("href"));
					if (m.find()) {
						String pageCount = m.group(1);
//						pageCount = (int) Math.ceil(Integer.parseInt(pageCount) / 50.0) + "";
