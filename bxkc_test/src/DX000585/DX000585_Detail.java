...
public class DX000585_Detail extends TxtRspHandler {
...
//				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
				String path = "http://www.specl.com.cn";
				Document xml = Jsoup.parse(originTxtRspContent);
				xml.outputSettings().prettyPrint(true);
				
				Element titleTag = xml.select("div[class=xinwen-head text]").first().select("h1").first();
				String title = titleTag.text().trim();
				response.title = title;
				
				//取正文内容
				Element conTag = xml.select("div[class=xinwen-main]").first();
