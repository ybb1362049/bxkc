 ...
 *
 */
public class SJ_04427_List extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == rsp) {
			try {
				...
				// 具体赋值
				String syntaxOfRows = "{tr>td[class=name]>a[href]}::<3";//span:matches(20\\d{2}-\\d{1,2}-\\d{1,2})
				String syntaxOfRows_ = "";
				String[] syntaxesOfRowFilter = {}; // 过滤一些row，syntax如：a[class=xx],就会将该标签所在row过滤
				boolean filterateLastRow = false;
				...
				String regexOfTitle = "td[class=name]>a[href]";// 如果title能从a标签中获取就不需要配置该语法
				String regexOfDate = "td";
				String regexOfPage = "{table>tbody>tr>td[align]:contains(页)}::<1";
				...
				// re_rows
				Element rowHtml = null;
				if(document.select("table[class=tables_01]")!=null){
					rowHtml = document.select("table[class=tables_01]").first();
				}
				System.out.println(rowHtml);
				Elements rows = getSameRows(rowHtml, syntaxOfRows);
				...
				OUTFOR: for (Element row : rows) {
					...
					if (getDateFlag) {
						...
						String date = resolveDate(date_htmls);
	...
	public static String resolveDate(Elements date_htmls) {
		String date = null;
