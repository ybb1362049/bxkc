...
public class XX0943_List extends TxtRspHandler {
	...
	Pattern page = Pattern.compile("/(\\s*)(\\d+)(\\s*)页");
	Matcher m;
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element dive = doc.select("div[class=pagination]").last();
				if (dive != null) {
				...
				Element ul = doc.select("div.c1-body").first();
				if (ul != null) {
					Elements list = ul.select("div.c1-bline");
					for (Element li : list) {
