...
public class SJ_04336_List extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element table = xml.select("div[class=InRight]").first();
				Elements trs = table.select("h2:has(a)");
				for(Element tre : trs){
...
//					String title = ae.text().trim().replace("...", "");
					String title = ae.text().trim().replace("...", "");
					String date = tre.select("span[class=hui999]").first().text().trim().replace("[", "").replace("]", "");
				...
				Element str = xml.select("div[class=page]").first();
