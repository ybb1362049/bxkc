...
public class DS_00490_List3 extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element div = doc.select("div[class=mr-content]").last();
				if (div != null) {
					...
					for (int i = 0; i < list.size(); i++) {
						...
						bn.url = super.getNewPathPrefix() + "/?" + super.getAdditionalLinkParamStr()
								+ "&iw-cmd=DS_00490_Detail3&" + id;
						String title = a.attr("title").trim();
						bn.id = id;
						bn.date = date;
						bn.title = title.replaceAll("\\[(.*?)\\]|\\s*", "").replace("??", "");
