...
public class SJ_00171_List extends TxtRspHandler {
...
//				originTxtRspContent="<html><body>"+originTxtRspContent+"<\body>"+"<"+"\\"+"html>";
				...
				NodeList div_list=xmlDoc.getElementsByTagName("UL");
				for(int j=0;j<div_list.getLength();j++){
				Element div=(Element) div_list.item(j);
				if("Expand_SearchSLisi".equals(div.getAttribute("class"))){
...
//							////////////System.out.printlnn(href);
							String InfoID=href.substring(href.indexOf("article/")+8,href.indexOf("."));
...
//							Element div2=(Element) div_list.item(j+2);
							Element SUP=(Element) ul_ele.getElementsByTagName("P").item(0);
							date=SUP.getTextContent().trim();
//							//////////System.out.printlnn(date);
							date=date.substring(date.indexOf("发布时间：")+5,date.indexOf("发布时间：")+15);
...
//				page=td_ele.getTextContent().trim();
				NodeList div_list1=xmlDoc.getElementsByTagName("SPAN");
				for(int i=0;i<div_list1.getLength();i++){
					Element td_ele=(Element) div_list1.item(i);
					if("pagecss".equals(td_ele.getAttribute("class"))){
						page=td_ele.getTextContent();
						page=page.substring(page.indexOf("总")+1,page.indexOf("篇")).trim();
