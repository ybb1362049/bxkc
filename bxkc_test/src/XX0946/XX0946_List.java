...
public class XX0946_List extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element dive = doc.select("td[id=fanye125471]").last();
				if (dive != null) {
				...
				Element ul = doc.select("tr[id=line_u1_0]").first();
				if (ul != null) {
					Element tbody=ul.parent();
					tbody.select("table").remove();
					Elements list = tbody.select("tr:has(a)");
					for (Element li : list) {
