...
public class XX1301_Detail1 extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState repState,
		...
		if (repState.equals(RspState.Login)) {
			try {
				...
				Element content = doc
						.select("form[name=_newscontent_fromname]").first();
				if(doc.text().length()<20){
					response.content = "该网页为无效详情跳过抓取";
				}
				if (content != null) {
					content.select("p:contains(新闻来源)").last().remove();
					content.select("a:contains(关闭)").last().remove();
