...
public class DS_00578_List extends TxtRspHandler {
	...
	private static Pattern page = Pattern.compile("/(\\d*)页");
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element dive = doc.select("div.pages").first();
				if (dive != null) {
				...
				Element div = doc.select("div[class=new-list height410]").first();
				if (div != null) {
					...
					for (int i = 0; i < list.size(); i++) {
						...
						bn.url = super.getNewPathPrefix() + "/?" + super.getAdditionalLinkParamStr()
								+ "&iw-cmd=DS_00578_Detail&iw_ir_1=" + id;
						m = p.matcher(li.text());
						String date = "";
						if (m.find()) {
