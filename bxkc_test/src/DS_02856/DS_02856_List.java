...
public class DS_02856_List extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element dive = doc.select("div.show_page").first();
				if (dive != null) {
				...
				Element div = doc.select("table[class=style1]").first();
				if (div != null) {
					...
					for (int i = 0; i < list.size(); i++) {
						...
						bn.url = super.getNewPathPrefix() + "/?" + super.getAdditionalLinkParamStr()
								+ "&iw-cmd=DS_02856_Detail&iw_ir_1=" + id;
						m = p.matcher(li.text());
						String date = "";
						if (m.find()) {
