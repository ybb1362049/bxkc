...
public class DX000571_Detail extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				String remove = "table#Table5 tr:contains(打印),-1";
				boolean getdate = false;
				String dateString = "";

				Document xml = Jsoup.parse(originTxtRspContent);
				Element titElement = xml.select("span#ucNewsView_lblMainTitle").first();
				Element content = xml.select("table#Table3").first();
