...
public class DX000571_List extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				String remove = "td:has(select#listPage)";

				Element content = xml.select("table#Table1").get(1);
				content= content.select("table:not(#Table1)").first();
				removeChild(content, remove);
				Elements list = content.select("tr:has(a)");

				for (Element value : list) {

					Element aElement = value.select("a").first();
					Element dateElement = value.select("font").last();
