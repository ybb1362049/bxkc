...
public class DS_00275_List extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element content = xml.select("div.list_r_box").first();
				...
				for (Element value : list) {
					...
					if (!aElement.attr("href").contains("http://www.cnbz.gov.cn")) {
						continue;
					}
					...
					bn.id = getHrefString(aElement, "gov.cn");
