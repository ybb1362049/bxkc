...
public class QX_01095_Detail extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				String remove = "div#divbz font.webfont";
				boolean getdate = true;
				String dateString = "font.webfont";

				Document xml = Jsoup.parse(originTxtRspContent);
				Element titElement = xml.select("span#literTitle").first();
				Element content = xml.select("div#dContent").first();
				if (getdate) {
