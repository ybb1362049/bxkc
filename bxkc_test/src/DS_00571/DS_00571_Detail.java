...
public class DS_00571_Detail extends TxtRspHandler {
...
//				}
				...
				Element conTag = xml.select("table[width=99%]").first();
				...
				Element uselessTag = conTag.select("div[class=artSource]").first();
				if(uselessTag != null){
					uselessTag.remove();
				}
				Element uselessTag1 = conTag.select("td[class=artBottom]").first();
				if(uselessTag1 != null){
					uselessTag1.remove();
				}
