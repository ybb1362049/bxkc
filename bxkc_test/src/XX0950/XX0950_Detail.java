...
public class XX0950_Detail extends TxtRspHandler {
	...
	String url = "http://hqc.cqcvc.com.cn";
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				String content = "";
				Element div = doc.select("div[id=Article]").first();
				if (div != null) {
					div.select("script").remove();
					div.select("input").remove();
					Element c=div.select("h1").first();
					if(c!=null)
					{
						c.select("span").remove();
						content=c.outerHtml();
					}
					div=div.select("div.content").first();
