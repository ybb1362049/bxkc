...
public class DX000578_Detail extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				String remove = "div.right-top,0 div#ctl00_ContentPlaceHolder1_news_ctl00_proInfo div.part-item-in";
				boolean getdate = true;
				String dateString = "div#ctl00_ContentPlaceHolder1_news_ctl00_proInfo";

				Document xml = Jsoup.parse(originTxtRspContent);
				Element titElement = xml.select("#ctl00_ContentPlaceHolder1_news_ctl00_proTit").first();
				Element content = xml.select("div[class=right fr]").first();
				if (getdate) {
