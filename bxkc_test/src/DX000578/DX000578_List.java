...
public class DX000578_List extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element content = xml.select("ul.right-news").first();
				...
				for (Element value : list) {
					...
					Element dateElement = value.select("div.date").last();
	...
	public String getDate(Element dateElement) throws ParseException {
		...
		String regex = "(?<day>\\d{1,2})(\\D)(?<year>20\\d{2}|\\d{2})(\\D)(?<month>\\d{1,2})";
		Pattern p = Pattern.compile(regex);
		Matcher m = p.matcher(dateElement.text().trim());
