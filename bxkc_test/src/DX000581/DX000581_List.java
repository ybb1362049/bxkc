...
public class DX000581_List extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element content = xml.select("ul.news_ul").first();
				...
				for (Element value : list) {
					...
					Element dateElement = value.select("strong").last();

					BranchNew bn = new BranchNew();
					bn.title = getTitle(aElement).replace("...", "");
