...
public class DS_01402_Detail extends TxtReqRspHandler {
	...
	private static String url = "http://www.cdfgj.gov.cn";
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				String content = "";
				Element td = doc.select("span[id=ID_ucShowNews_UcNewsView1_lbMainTitle]").first();
				if (td != null) {
					response.title=td.text().trim();
					content = td.outerHtml();
				}
				Element div = doc.select("span.newscontent").last();
				if (div != null) {
