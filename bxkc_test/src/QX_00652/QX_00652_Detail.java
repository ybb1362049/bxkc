...
public class QX_00652_Detail extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element detailDivEle = document.select("td[id=TDContent]").first();
				if(detailDivEle.text().isEmpty()){
					response.content="该网站还没有更新内容";
					return response;
				}
