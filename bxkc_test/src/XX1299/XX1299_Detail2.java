...
public class XX1299_Detail2 extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState repState, String originTxtRspContent) {
		...
		if (repState.equals(RspState.Login)) {
			try {
				...
				Element content = doc.select("div.neirong_right2").first();
				if(content != null){
					content.select("#xw_xinxi").remove();
					content.select("table:contains(上一篇)").last().remove();
