...
public class DX000568_List extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element content = xml.select("div.cy_box_l").first();
				removeChild(content, remove);
				Elements list = content.select("a:has(div.ten_cen_one)");

				for (Element value : list) {

					Element aElement = value.select("a").first();
					Element titEL = value.select("p.ten_cen_r_time").first();
					Element dateElement = value.select("p.ten_cen_r_h1").last();

					BranchNew bn = new BranchNew();
					bn.title = getTitle(titEL);
