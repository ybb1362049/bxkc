...
public class DX000584_Detail extends TxtRspHandler {
...
//				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
				String path = "http://www.ymlgs.com.cn";
				Document xml = Jsoup.parse(originTxtRspContent);
				xml.outputSettings().prettyPrint(true);
				
				Element dive = xml.select("div[class=content]").first();
				
				Element titleTag = dive.select("div[class=title]").first().select("h3").first();
				String title = titleTag.text().trim();
				response.title = title;
				
				//取正文内容
				Element conTag = dive.select("div[class=maincontent clearfix]").first();
