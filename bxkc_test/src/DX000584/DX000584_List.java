...
public class DX000584_List extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == rsp) {
			try {
				...
				Element dive = xml.select("div[class=ArticleList]").first();
				Element table = dive.select("table[width=100%]").first();
				Elements lis = table.select("tr:has(a)");
				for(Element lie : lis){
					Element ae = lie.select("a").last();
...
//					id = id.replace("../", "");
					String url = super.getNewPathPrefix() + "/?" + super.getAdditionalLinkParamStr()
							+ "&iw-cmd=DX000584_Detail&iw_ir_1=" + id;
