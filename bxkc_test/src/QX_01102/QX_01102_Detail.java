...
public class QX_01102_Detail extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element titleElement = xml.select("h2.title").first();
				String title = titleElement.text().trim();
				response.title = title;
				Element contentElement = xml.select("div.conTxt").last();
				...
				if (contentElement.select("a").first() != null) {
					...
					for (Element href : as) {
						...
						if (!hre.contains("@") && !"".equals(hre) && !hre.contains("javascript") && !hre.contains("http")) {
							hre = "http://www.jiangyang.gov.cn" + hre;
							href.attr("href", hre);
						}
					}
				}
				if (contentElement.select("img").first() != null) {
					...
					for (Element imgs : img) {
						...
						if (!imgss.contains("javascript") && !"".equals(imgss) && !imgss.contains("http")) {
							imgss = "http://www.jiangyang.gov.cn" + imgss;
							imgs.attr("src", imgss);
