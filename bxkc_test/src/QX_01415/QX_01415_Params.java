...
public class QX_01415_Params extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element dive = doc.select("tr[class=myGVPagerCss]").first();
				if (dive != null) {
				...
				Element div = doc.select("table#ctl00_ContentPlaceHolder1_myGV").last();
				if (div != null) {
					...
					for (int i = 0; i < list.size(); i++) {
						...
						bn.url = super.getNewPathPrefix() + "/?" + super.getAdditionalLinkParamStr()
								+ "&iw-cmd=QX_01415_Detail&iw_ir_2=" + id;
				...
				if (doc.getElementById("__VIEWSTATE") != null) {
					...
					response.nextPage = super.getNewPathPrefix() + "/?" + super.getAdditionalLinkParamStr()
							+ "&iw-cmd=QX_01415_List&" + aspnetForm + "&__VIEWSTATE=" + __VIEWSTATE
