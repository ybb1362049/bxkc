...
public class QX_01645_List extends TxtRspHandler {
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element content = xml.select("div.informationList").first();
				removeChild(content, remove);
				Elements list = content.select("li.inforTitle:has(a)");

				for (Element value : list) {

					Element aElement = value.select("a").first();
					Element dateElement = value.nextElementSibling().nextElementSibling();

					BranchNew bn = new BranchNew();
					bn.title = getTitle(aElement);
					bn.date = getDate(dateElement);
					bn.id = getHrefString(aElement, ".cn");
