...
public class QX_01093_List extends TxtRspHandler {
	...
	private static Pattern page = Pattern.compile("共(\\s*)(\\d+)(\\s*)页");
	...
	protected TxtRspObject processTxtRspContent(RspState rspState, String originTxtRspContent) {
		...
		if (rspState == RspState.Login) {
			try {
				...
				Element dive = doc.select("div#page_div").first();
				if (dive != null) {
				...
				Element div = doc.select("div[class=list]").first();
				if (div != null) {
					...
					if (ul != null) {
						...
						for (int i = 0; i < list.size(); i++) {
							...
							id = id.substring(id.lastIndexOf("/pidu"));
							bn.url = super.getNewPathPrefix() + "/?" + super.getAdditionalLinkParamStr()
									+ "&iw-cmd=QX_01093_Detail&iw_ir_1=" + id;
							m = p.matcher(li.text());
							String date = "";
							if (m.find()) {
